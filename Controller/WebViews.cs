...
namespace Control
{
    ...
    public static class WebViews
    {
        ...
        /// <returns></returns>
        public static string GetAllGames(Dictionary<int, GameModel> games)
        {
            StringBuilder sb = new StringBuilder();

            foreach (int gid in games.Keys)
            {
                sb.Append("Game " + gid + " (" + games[gid].Duration/1000 + " seconds)<br>");
                sb.Append("<table border=\"1\">");
                sb.Append("<tr><th>Name</th><th>Score</th><th>Accuracy</th></tr>");
                foreach (PlayerModel p in games[gid].GetPlayers().Values)
                {
                    sb.Append("<tr>");
        ...
        public static string GetPlayerGames(string name, List<SessionModel> games)
        {
            ...
            foreach (SessionModel s in games)
            {
                ...
                sb.Append("<td>" + s.Duration/1000 + "</td>");
